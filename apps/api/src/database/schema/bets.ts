...
export const bets = pgTable(
  'bets',
  {
    ...
    /** `null` en `PENDING` y en `LOST` (D-B2); obligatorio en `WON`/`VOID`/`CASHOUT`. */
  ...
  (table) => [
    ...
    check(
      ...
      sql`(${table.status} = 'PENDING'
            AND ${table.settledAt} IS NULL AND ${table.officialRealizedReturn} IS NULL)
          OR (${table.status} = 'LOST'
            AND ${table.settledAt} IS NOT NULL AND ${table.officialRealizedReturn} IS NULL)
          OR (${table.status} IN ('WON', 'VOID', 'CASHOUT')
            AND ${table.settledAt} IS NOT NULL
            AND ${table.officialRealizedReturn} IS NOT NULL
            AND ${table.officialRealizedReturn} >= 0)`,
    ),
    check(
